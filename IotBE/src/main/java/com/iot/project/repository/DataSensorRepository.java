...
public interface DataSensorRepository extends JpaRepository<DataSensor, Long> {
   ...
   SELECT d.id AS id,
          ...
          d.time AS time
   FROM DataSensor d
   WHERE (:search IS NULL OR d.temperature LIKE %:search% OR d.humidity LIKE %:search% OR d.light LIKE %:search%)
   AND (:startDate IS NULL OR d.time >= :startDate)
   AND (:endDate IS NULL OR d.time <= :endDate)
""")
    Page<DataSensorProjection> findDataSensorsWithCondition(String search, LocalDateTime startDate, LocalDateTime endDate, Pageable pageable);
